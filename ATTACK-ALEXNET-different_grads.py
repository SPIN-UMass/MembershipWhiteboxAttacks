...
def attack(trainloader,testloader, model,inference_model,classifier_criterion,classifier_criterion_noreduct, criterion,classifier_optimizer ,optimizer, epoch, use_cuda,num_batchs=1000,is_train=False):
    global best_acc
    ...
    for batch_idx,((tr_input, tr_target) ,(te_input, te_target)) in enumerate(zip(trainloader,testloader)):
        ...
        c= classifier_criterion_noreduct(pred_outputs,infer_input).view([-1,1])#torch.autograd.Variable(torch.from_numpy(c.view([-1,1]).data.cpu().numpy()).cuda())
        
        
        
        preds = torch.autograd.Variable(torch.from_numpy(pred_outputs.data.cpu().numpy()).cuda())
        member_output = inference_model(grads,infer_input_one_hot,c,pred_outputs,l1,l2)
...
inferenece_model = InferenceAttack_HZ(100,num_layers=0)
...
for epoch in range(100):
    attack(private_trainloader_intrain,private_trainloader_intest,net,inferenece_model,criterion,criterion_classifier,criterion_attack,optimizer,optimizer_mem,epoch,use_cuda,is_train=True)
    bb=attack(private_testloader_intrain,private_testloader_intest,net,inferenece_model,criterion,criterion_classifier,criterion_attack,optimizer,optimizer_mem,epoch,use_cuda,is_train=False)
